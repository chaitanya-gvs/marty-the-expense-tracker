...
// Inline date editor — shows a hover-reveal pencil icon that opens a date popover
function InlineDateCell({ transaction }: { transaction: Transaction }) {
  const [open, setOpen] = useState(false);
  const [value, setValue] = useState(transaction.date);
  const updateTransaction = useUpdateTransaction();

  const handleSave = async () => {
    if (value === transaction.date) { setOpen(false); return; }
    try {
  ...
  return (
    <div className="flex items-center gap-1">
      {transaction.original_date && (
        <span
          title={`Original: ${transaction.original_date}`}
        ...
        <PopoverContent className="w-auto p-3 space-y-2" align="start">
          ...
          <input
            ...
            onKeyDown={(e) => {
              ...
              if (e.key === "Escape") { setValue(transaction.date); setOpen(false); }
            }}
            autoFocus
          />
          <div className="flex gap-1.5 justify-end">
            <Button type="button" variant="ghost" size="sm" className="h-7 text-xs" onClick={() => { setValue(transaction.date); setOpen(false); }}>Cancel</Button>
            <Button type="button" size="sm" className="h-7 text-xs" onClick={handleSave} disabled={updateTransaction.isPending}>Save</Button>
          </div>
        </PopoverContent>
      </Popover>
    </div>
