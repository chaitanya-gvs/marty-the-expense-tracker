...
- Primary "Edit" button
- A 7-tile action grid — Split, Group, Flag, **Swap direction**, Links, PDF, Recurring — plus an isolated Delete row below a divider. This is the *complete* action set (unlike the panel's reduced 8-tile set in Component 5, this grid adds the direction-toggle action and excludes Edit/Select, since Edit is already the primary button above and Select doesn't apply — you're already viewing one specific transaction, not a list). Rationale for having a full action set here at all, not just in the long-press panel: the drawer is reached by a *plain tap*, the primary/expected way to open a transaction on mobile — it needs to be a complete hub on its own, not depend on the user knowing long-press exists.
