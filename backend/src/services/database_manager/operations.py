...
# ---------------------------------------------------------------------------
# Statement Processing Log operations
# ---------------------------------------------------------------------------
...
class StatementLogOperations:
    ...
    async def upsert_log(data: dict) -> dict:
        ...
        try:
            ...
            await session.rollback()
            logger.error(f"Error upserting statement log for {data.get('normalized_filename')}", exc_info=True)
    ...
    async def update_status(normalized_filename: str, status: str, **extra_fields) -> bool:
        ...
        try:
            ...
            await session.rollback()
            logger.error(f"Error updating status for {normalized_filename}", exc_info=True)
            return False
        finally:
            await session.close()
    ...
    async def set_error(normalized_filename: str, error: str) -> bool:
        ...
        try:
            ...
            await session.rollback()
            logger.error(f"Error setting error for {normalized_filename}", exc_info=True)
            return False
        finally:
            await session.close()
    ...
    async def check_already_extracted(normalized_filename: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error checking extraction status for {normalized_filename}", exc_info=True)
            return False
        finally:
            await session.close()
    ...
    async def get_incomplete_statements(statement_month: str) -> List[dict]:
        ...
        try:
            ...
            logger.error(f"Error fetching incomplete statements for {statement_month}", exc_info=True)
            return []
        finally:
            await session.close()
